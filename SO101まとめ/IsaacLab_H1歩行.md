...
本ノートは **Isaac Lab の導入とシミュレーション内での学習**まで。実機への展開は別ノート。

| 関連ノート | 内容 |
|---|---|
| [[SO101_現状まとめ_20260909]] | **最上位の現状まとめ**（GR00T／Isaac Lab の2トラック） |
| [[IsaacLab_SO101_実機化]] | このノートの続き。**SO-101実機への展開** |
| [[GR00T　2026-09-07]] | SO-101 模倣学習側（トラックA） |
| [[Clippings/第14章　Isaac SimによるReal-to-Simシミュレーション]] | 理論的背景 |
...
**Go2（四足・平地）**
- **学習過程（40秒）**：![[その他/go2_学習過程_20260830.mp4]] ← 未学習→転倒→歩容形成→完成
- 30秒版（25体）：![[その他/go2_flat_20260830.mp4]]

**H1 Rough（二足・凹凸・階段）**
- **学習過程（40秒）**：![[その他/h1_rough_学習過程_20260831.mp4]] ← 地形カリキュラムの降格→上昇→上限→習熟
...
> **注意**：`save_interval = 50` なので**チェックポイントは 0/50/100/150/200/250/299 のみ**。
> 上表の 25・75 はログ上の値であって、その時点のモデルは保存されていない。
> 学習過程の動画で「反転の瞬間（75）」を使いたい場合は、
> 事前に `--save_interval 25` 等を指定して学習し直す必要がある。
> 今回の学習過程動画は代わりに **0 / 50 / 150 / 299** の4点で構成した。
...
端末を別のネットワークに移すと Tailscale が遮断されることがある。切り分け手順：

```bash
ping -n 2 8.8.8.8                      # ① 一般インターネットは生きているか
tailscale status                        # ② rx 0 なら受信できていない
tailscale netcheck                      # ③ "UDP: false" なら UDP遮断
curl -o /dev/null -w "%{http_code}\n" https://derp9.tailscale.com/   # ④ 000 ならDERPも遮断
```

Tailscale の通信経路は **①UDP直接接続** と **②DERPリレー（TCP/443）** の2つだけ。
**両方塞がれていると打つ手がない**（調整サーバーに到達できても意味がない）。

| 症状の組合せ | 判定 |
|---|---|
| 一般ネットOK・UDP false・DERP 000 | **そのネットワークがTailscaleを遮断**。テザリング等に切り替えるしかない |
| 一般ネットNG | 単なるオフライン |
| UDP false・DERP到達可 | リレー経由で低速だが通信可能 |

実例では `gw=10.18.0.1` の別ネットワーク（大学構内等）でUDPとDERPの両方が塞がれていた。
**DGX Spark 側は無ダメージ**で、学習済みモデルもtmuxジョブもそのまま残る。
長時間ジョブは必ず tmux で走らせておくこと（SSH断で死なない）。
...
| ~~Rough の学習過程を動画化~~ | ✅ **2026-08-31 完了**（上記 動画リンク） |
| Go2 の「反転の瞬間」を撮る | `--save_interval 25` で再学習（4分）→ `model_75` を録画。現在の保存間隔50では撮れない |
| Rough を Go2 で回す | `noise_step` を 0.02〜0.03 に緩める必要あり（下記の重さ問題） |
